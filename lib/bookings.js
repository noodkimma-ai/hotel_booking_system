...
export async function updateBookingStatus(id, status) {  //yesma kun id ko status update garney 
  try {
    const token = localStorage.getItem("token");
    const res = await axios.patch(`http://localhost:5000/api/bookings/${id}/status`,
      {
        status:status
      },{
        headers:{
    ...
    console.log("Error update booking status",error);
    console.log("Error Response: ", error.response?.data);
